...
# News On AIR feed — Podcast & Full-Text (unofficial)

Self-hosted, consolidated **podcast and full-text English RSS 2.0 feed** of
[News On AIR](https://newsonair.gov.in) (All India Radio / Prasar Bharati)
bulletins and news magazines, built by `newsonair_feed.py`:
...
News On AIR's news **bulletins and magazines** (Morning / Midday / Evening / Parikrama / Aaj Savere —
AIR's signature content) have **no working official feed**, so there is no way to subscribe to audio broadcasts or full transcripts in standard podcast apps or RSS readers without reconstructing the feed.
...
```bash
pip install -r requirements.txt
python newsonair_feed.py
# -> public/newsonair/feed.xml
```

The GitHub Actions workflow rebuilds the consolidated podcast feed and deploys to GitHub Pages.

## Caveats

- Audio broadcasts are only retained by News On AIR for recent days; older archived episodes in history retain their complete transcripts.
...
3. `archive_pdfs.py --verify` performs the global reference verification check across all `/releases/download/<tag>/<filename>` links in published feeds against GitHub release inventories.
...
| `ARCHIVE_VERIFY_ONLY` | – | Set to `true`/`1` to run reference verification instead of uploading |
...
# verify feed references across all releases:
python archive_pdfs.py --verify
```
...
Full-text RSS feed for Swaminomics columns from
...
Swaminathan S. Anklesaria Aiyar's columns appear on TOI Plus under author profile
`18032`, without an official full-text RSS feed.

## How it works

- `toi.py` parses `window.App.state.toiplusauthor` from
...
- The body is sanitized: ad embeds and misc video widgets are stripped, and
  occasional content images are rendered as responsive `<figure>` tags hotlinking
  directly to the TOI CDN without archiving.

## Configuration (env vars)

| Var | Default | Meaning |
|-----|---------|---------|
| `TOI_MAX_FETCH` | `30` | Max new articles fetched per run |
...
- Depends on the AUFS article endpoint and TOI's author state JSON structure.
